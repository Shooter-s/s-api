...
import com.shooter.sapibackend.mapper.PostFavourMapper;
...
import com.shooter.sapibackend.mapper.PostMapper;
...
import com.shooter.sapibackend.utils.SqlUtils;
...
public class PostServiceImpl extends ServiceImpl<PostMapper, Post> implements IPostService {
    ...
    public QueryWrapper<Post> getQueryWrapper(PostQueryRequest postQueryRequest) {
        ...
        // 拼接查询条件
        if (StringUtils.isNotBlank(searchText)) { // searchText可以搜索到title或者content的内容
            queryWrapper.like("title", searchText).or().like("content", searchText);
        }
        queryWrapper.like(StringUtils.isNotBlank(title), "title", title);
        queryWrapper.like(StringUtils.isNotBlank(content), "content", content);
        if (CollUtil.isNotEmpty(tagList)) {
            for (String tag : tagList) { // 存在mysql中的是json类型，所以要这样拼接
    ...
    public Page<PostVO> getPostVOPage(Page<Post> postPage, HttpServletRequest request) {
        ...
        Map<Long, List<User>> userIdUserListMap = userService.listByIds(userIdSet).stream() // map   id： 用户信息
